return {
	...
	config = function(_, opts)
		...
		wk.add({
			...
			{ "<leader>nww", "<cmd>Neorg workspace poetry<cr>", desc = "Poetry" },
			{ "<leader>nww", "<cmd>Neorg workspace gilroy<cr>", desc = "Gilroy" },
			...
			{ "<leader>njw", "<cmd>Neorg workspace poetry <CR><cmd> Neorg journal today<cr>", desc = "Poetry" },
			{ "<leader>njw", "<cmd>Neorg workspace gilroy <CR><cmd> Neorg journal today<cr>", desc = "Gilroy" },
		})
		...
		if require("zk.util").notebook_root(vim.fn.expand("%:p")) ~= nil then
			...
			map("n", "<CR>", "<Cmd>lua vim.lsp.buf.definition()<CR>", options)
			...
			map("v", "<leader>znt", ":'<,'>ZkNewFromTitleSelection { dir = vim.fn.expand('%:p:h') }<CR>", options)
			...
			map("n", "<leader>zb", "<Cmd>ZkBacklinks<CR>", options)
			-- Alternative for backlinks using pure LSP and showing the source context.
			--map('n', '<leader>zb', '<Cmd>lua vim.lsp.buf.references()<CR>', options)
			-- Open notes linked by the current buffer.
			map("n", "<leader>zl", "<Cmd>ZkLinks<CR>", options)

			-- Preview a linked note.
			map("n", "K", "<Cmd>lua vim.lsp.buf.hover()<CR>", options)
			-- Open the code actions for a visual selection.
			map("v", "<leader>za", ":'<,'>lua vim.lsp.buf.range_code_action()<CR>", options)
		end
	end,
	setup = function()
